...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.ref_head = nn.Conv2d(64, model_channels, 3, 1, 1)
        self.conv = nn.Conv2d(model_channels, model_channels, 3, 1, 1)
        self.act = nn.PReLU()
        ...
        self.input_ref_blocks = nn.ModuleList()    
        self._feature_size = model_channels
        input_block_chans = [model_channels]
        input_ref_block_chans = [model_channels]
        ch = model_channels
        ds = 1
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                layers = [
                    ResnetBlock(
                        ch, mult * model_channels, dropout=dropout)
                ]
                ch = mult * model_channels
                self.input_ref_blocks.append(*layers)
                self._feature_size += ch
                input_ref_block_chans.append(ch)
            if level != len(channel_mult) - 1:
                out_ch = ch
                self.input_ref_blocks.append(
                    ResnetBlock(
                        ch, mult * model_channels, dropout=dropout)
                )
                ch = out_ch
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        AttentionBlock(
                            ...
                            ch, num_heads, dim_head, image_size, depth=transformer_depth, context_dim=context_dim
                        )
                    )
        ...
        self.up1 = D_UpBlock(model_channels, kernel_size=6, stride=2, padding=2)
        self.up2 = D_UpBlock(model_channels, kernel_size=6, stride=2, padding=2)
        self.max_pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv_256 = nn.Conv2d(128, 256, 3, 1, 1)
        self.conv_512 = nn.Conv2d(256, 512, 3, 1, 1)
    ...
    def forward(self, x, timesteps=None, context=None, y=None,**kwargs):
        ...
        h = x.type(self.dtype)
        ref = context.type(self.dtype)
        ref = self.ref_head(ref)

        # for module_ref, module in zip(self.input_ref_blocks, self.input_blocks):
        #     ref = module_ref(ref)
        #     h = module(h, emb, ref) 
        #     hs.append(h)
        for module in self.input_blocks:
            h = module(h, emb, ref)
            # ref = self.max_po ol(ref)
            hs.append(h)

        # ref_down = self.act(self.conv(self.max_pool(ref)))
        h = self.middle_block(h, emb, ref)
