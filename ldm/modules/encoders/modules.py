...
class EDSR(nn.Module):
    def __init__(self, conv=default_conv, 
        ...
        #     nn.Sequential(
        #         ResBlock(conv, self.n_feats, 5, act=act),
        #         ResBlock(conv, self.n_feats, 5, act=act)
        #     ) 
        # ])
        ...
        # m_body = [
        #     ResBlock(
        #         conv, self.n_feats, kernel_size, act=act
        #     ) for _ in range(n_resblocks)
        # ]
        ...
        self.to_q_global_3 = nn.Sequential(
                FeatExtract(self.n_feats, keep_dim=False),
            )
        self.conv3 = nn.Conv2d(64, self.n_feats, 3, 1, 1)
        

    def forward(self, x):
        in_features = self.ref_feat(x)
        x = self.conv3(in_features['relu1_1'])
    #     x = self.sub_mean(x)
    #     x = self.head(x)
...
class SpatialRescaler_(nn.Module):
    def __init__(self,
                 n_stages=1,
                 method='bilinear',
                 multiplier=0.5,
        ...
        self.ref_feat = CorrespondenceGenerationArch()
        # self.to_q_global_3 = nn.Sequential(
        #         FeatExtract(n_feats, keep_dim=False),
        #     )
        ...
        if self.remap_output:
            ...
            self.channel_mapper = nn.Conv2d(n_feats,out_channels,1,bias=bias)

    def forward(self,x):
        in_features = self.ref_feat(x)
        x = in_features['relu1_1']
...
class NewModel(torch.nn.Module):
    def __init__(self, pretrained_model):
        ...
        self.features = torch.nn.Sequential(*list(pretrained_model.children())[:-5])
...
class FrozenClipImageEmbedder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # pretrained_model = models.resnext50_32x4d(pretrained=True).cuda()
        # self.model = NewModel(pretrained_model)
        # self.conv = nn.Conv2d(256, 3, 3, 1, 1)

        model, _ = clip.load(name=model, device=device, jit=jit)
        self.model = NewModel(model)
    ...
    def forward(self, x):
        ...
        # print ('clip', self.model(self.preprocess(x)).shape)
...
class CorrespondenceGenerationArch(nn.Module):
    ...
    def __init__(self,
                 patch_size=3,
                 stride=1,
                 vgg_layer_list=['relu3_1','relu1_1'],
                 vgg_type='vgg19'):
